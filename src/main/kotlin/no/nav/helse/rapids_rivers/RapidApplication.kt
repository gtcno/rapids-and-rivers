        ...
        companion object {
            fun fromEnv(env: Map<String, String>) = generateInstanceId(env).let { instanceId ->
                RapidApplicationConfig(
                    ...
                    ?: log.info("app name not configured").let { null },
            ...
            private fun generateAppName(env: Map<String, String>): String? {
                val appName = env["NAIS_APP_NAME"]
                    ?: return log.info("not generating app name because NAIS_APP_NAME not set").let { null }
                val namespace = env["NAIS_NAMESPACE"]
                    ?: return log.info("not generating app name because NAIS_NAMESPACE not set").let { null }
